import { OPENING_HOURS_INTERVAL, now } from '@/components/Calendar/Constants/config'
...
export const getOpeningTimes = (startDate: Date, days: Day[]) => {
    ...
    if (!today) {
      console.warn(`No entry found for dayOfWeek ${dayOfWeek}`);
      return []; // Return an empty array or handle the case as per your application logic
    }
